...
let db = null;
const appId = 'twokinp-production-v17';
...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-white text-gray-900 font-sans selection:bg-[#FFC107]/30">
            ...
            {SERVICES_DATA.map(cat => (
                <div key={cat.category} className="relative group text-black">
                    ...
                    <div className="absolute top-full left-0 w-64 bg-white shadow-2xl rounded-xl py-4 border opacity-0 invisible group-hover:opacity-100 group-hover:visible translate-y-4 group-hover:translate-y-2 transition-all">
                        {cat.products.map(p => (<button key={p} onClick={() => {setFilter(cat.category); setIsQuoteModalOpen(true);}} className="w-full text-left px-6 py-2 text-[9px] font-black text-gray-400 hover:text-[#FFC107] uppercase tracking-widest">{p}</button>))}
                    </div>
                </div>
            ))}
        </nav>
      ...
      <main className="max-w-[1600px] mx-auto px-6 py-12 text-black">
        
        {/* AUTO SLIDER (3s) */}
        {!isAdminMode && banners.length > 0 && (
            <div className="mb-16 relative h-[550px] rounded-xl overflow-hidden shadow-xl bg-gray-50 border border-gray-100">
                {banners.map((s, i) => (
                    <div key={i} className={`absolute inset-0 transition-opacity duration-1000 ${i === currentSlide ? 'opacity-100' : 'opacity-0'}`}>
                        ...
                        <div className="absolute inset-0 flex items-center px-12 sm:px-24 text-left">
                            <div className="max-w-xl animate-in slide-in-from-left">
                                ...
                                <h2 className="text-5xl sm:text-7xl font-black text-black leading-none uppercase tracking-tighter mb-8 italic">{s.title}</h2>
                                <p className="text-gray-500 text-lg mb-10 font-bold uppercase">{s.subtitle}</p>
                                <button onClick={() => setIsQuoteModalOpen(true)} className="bg-black text-white px-10 py-5 rounded-full font-black text-xs uppercase flex items-center gap-3 hover:bg-[#FFC107]">Start Project <ArrowRight/></button>
                            </div>
                        </div>
                    </div>
                ))}
        ...
        {!isAdminMode && videos.length > 0 && (
            <div className="mb-20 text-left">
                <h2 className="text-3xl font-black uppercase mb-10 tracking-tighter italic text-black">Featured <span className="text-[#FFC107]">Gallery</span></h2>
        ...
        <div ref={portfolioRef} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10">
            {filteredProducts.map(p => (
                <div key={p.id} onClick={() => setSelectedDetails(p)} className="group cursor-pointer text-left">
                    <div className="relative aspect-[3/4] rounded-xl overflow-hidden border shadow-sm mb-6 transition-all group-hover:shadow-2xl">
                        ...
                        <div className="absolute inset-0 bg-black/20 opacity-0 group-hover:opacity-100 transition-all flex items-center justify-center">
                             <div className="bg-white text-black px-8 py-2.5 rounded-full font-black uppercase text-[10px] shadow-2xl">See more info</div>
                        </div>
                    </div>
                    <div className="px-2">
      ...
      <footer className="bg-gray-50 py-24 px-12 border-t border-gray-100 text-left text-black">
        <div className="max-w-[1600px] mx-auto grid grid-cols-1 md:grid-cols-4 gap-16">
            ...
            <div>
                <h4 className="font-black uppercase text-[11px] mb-8 tracking-[0.2em]">Our Office</h4>
            ...
            <div>
                <h4 className="font-black uppercase text-[11px] mb-8 tracking-[0.2em]">Social Media</h4>
      ...
      {/* PRODUCT MODAL WITH PRICE */}
      {selectedDetails && (
          <div className="fixed inset-0 z-[150] flex items-center justify-center p-4 bg-black/95 backdrop-blur-md animate-in fade-in text-black">
              <div className="bg-white rounded-xl max-w-4xl w-full overflow-hidden flex flex-col md:flex-row shadow-2xl relative text-left">
                  ...
                  <div className="md:w-1/2 p-10 flex flex-col justify-between">
                      <div>
                          ...
                          <p className="text-gray-500 text-sm leading-relaxed mb-6">{selectedDetails.description || "Premium service by Twokinp."}</p>
                      </div>
                      <button onClick={() => window.open(`https://wa.me/${siteSettings.whatsapp}`, '_blank')} className="w-full bg-[#FFC107] text-black p-6 rounded-xl font-black uppercase text-xs shadow-xl hover:bg-black hover:text-white transition-all">Order this now</button>
                  </div>
              </div>
          </div>
      )}

      {/* FULL DASHBOARD WITH FORMS */}
      {isAdminMode && (
          <div className="fixed inset-0 z-[120] bg-white overflow-y-auto p-12 text-left text-black animate-in slide-in-from-bottom">
              <div className="max-w-6xl mx-auto">
                  ...
                  {/* FORM TAB: SETTINGS */}
                  {adminTab === "settings" && (
                      <form onSubmit={async (e) => { e.preventDefault(); await setDoc(doc(db, 'artifacts', appId, 'public', 'data', 'settings', 'config'), siteSettings); alert("Updated!"); }} className="grid grid-cols-1 md:grid-cols-2 gap-10 bg-gray-50 p-12 rounded-[2.5rem] border text-black">
                          <h3 className="md:col-span-2 text-xl font-black uppercase text-[#FFC107]">English Profile Settings</h3>
                          <div className="space-y-3"><label className="text-[10px] font-black uppercase text-gray-400">Logo Image Link</label><input className="w-full p-5 bg-white border rounded-2xl font-bold" value={siteSettings.logoUrl} onChange={e => setSiteSettings({...siteSettings, logoUrl: e.target.value})} /></div>
                          <div className="space-y-3"><label className="text-[10px] font-black uppercase text-gray-400">Official WhatsApp</label><input className="w-full p-5 bg-white border rounded-2xl font-bold" value={siteSettings.whatsapp} onChange={e => setSiteSettings({...siteSettings, whatsapp: e.target.value})} /></div>
                          <div className="space-y-3 md:col-span-2"><label className="text-[10px] font-black uppercase text-gray-400">About Our Company (Footer)</label><textarea className="w-full p-5 bg-white border rounded-2xl h-32 font-bold" value={siteSettings.aboutUs} onChange={e => setSiteSettings({...siteSettings, aboutUs: e.target.value})} /></div>
                          <div className="space-y-3"><label className="text-[10px] font-black uppercase text-gray-400">Copyright Statement</label><input className="w-full p-5 bg-white border rounded-2xl font-bold" value={siteSettings.copyright} onChange={e => setSiteSettings({...siteSettings, copyright: e.target.value})} /></div>
                          <button type="submit" className="md:col-span-2 bg-black text-white p-7 rounded-3xl font-black uppercase tracking-widest hover:bg-[#FFC107]">Apply Settings</button>
                      </form>
                  )}
                  ...
                  {adminTab === "projects" && (
                      <div className="space-y-12">
                          <form onSubmit={handleAddProduct} className="grid grid-cols-1 md:grid-cols-2 gap-8 bg-gray-50 p-12 rounded-[2.5rem] border text-black">
                              <h3 className="md:col-span-2 text-xl font-black uppercase text-[#FFC107] italic">{editingProjectId ? "Update Item" : "Publish New Item"}</h3>
                              ...
                              <input placeholder="Image URL (Direct link)" className="p-5 rounded-2xl border font-bold text-blue-500" value={newProduct.image} onChange={e => setNewProduct({...newProduct, image: e.target.value})} required />
                              ...
                              <textarea placeholder="Service Description (English)" className="p-5 rounded-2xl border font-bold md:col-span-2 h-24 text-black" value={newProduct.description} onChange={e => setNewProduct({...newProduct, description: e.target.value})} />
                              <button type="submit" className="md:col-span-2 bg-black text-white p-6 rounded-2xl font-black uppercase tracking-widest hover:bg-[#FFC107]">{editingProjectId ? "Confirm Update" : "Create Project"}</button>
                          </form>
                          <div className="grid grid-cols-1 sm:grid-cols-5 gap-6">
                              {products.map(p => (<div key={p.id} className="bg-white p-4 rounded-3xl border relative group text-black">
                                  <img src={p.image} className="w-full h-32 object-cover rounded-2xl mb-4" />
                                  <h4 className="font-black uppercase text-[9px] truncate">{p.name}</h4>
                                  <div className="flex gap-2 mt-4"><button onClick={() => { setEditingProjectId(p.id); setNewProduct({...p}); window.scrollTo(0,0); }} className="text-[#FFC107]"><Pencil size={18}/></button><button onClick={async () => { if(confirm("Delete?")) await deleteDoc(doc(db, 'artifacts', appId, 'public', 'data', 'products', p.id)); }} className="text-red-500"><Trash2 size={18}/></button></div>
                              </div>))}
                          </div>
                      </div>
                  )}
                  ...
                  {adminTab === "banners" && (
                      <div className="space-y-12">
                          <form onSubmit={handleAddBanner} className="grid grid-cols-1 md:grid-cols-2 gap-8 bg-gray-50 p-12 rounded-[2.5rem] border text-black">
                              ...
                              <button type="submit" className="md:col-span-2 bg-black text-white p-6 rounded-2xl font-black uppercase tracking-widest hover:bg-[#FFC107]">Save Slider</button>
                          </form>
                          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        <div className="fixed inset-0 z-[1000] flex items-center justify-center bg-white/95 backdrop-blur-xl animate-in fade-in text-black">
            <div className="w-full max-w-sm p-12 text-center text-black">
                ...
                <form onSubmit={(e) => {
                    ...
                    <button type="button" onClick={() => setIsPasswordModalOpen(false)} className="text-[10px] text-gray-300 uppercase font-black tracking-[0.3em] mt-10 hover:text-black transition-all underline underline-offset-8">Return Home</button>
                </form>
            </div>
        </div>
      )}
...
// TWOKINP V17.0 - STABLE DASHBOARD & CLEAN TOP